...
class FloodPipeline:
    ...
    def _run_tp2(self):
        ...
        seuil_db = p.get("seuil_db", -26.0)
        epsg     = p.get("epsg", 32629)
        out_mask = os.path.join(self.outdir, "mask_water.tif")

        graph_xml = self._write_snap_graph_tp2(img, aoi, pol, px, dem, seuil_db, epsg, out_mask)

        job.set_progress(10, "Exécution SNAP (Apply Orbit → TNR → Calibration → Speckle → TC → Seuillage)")
        success = self._run_snap_graph(graph_xml, label="TP2-SNAP")

        if not success or not os.path.exists(out_mask):
            raise RuntimeError("SNAP TP2 a échoué — vérifier les logs")

        job.results["mask_water"] = out_mask
        job.set_progress(45, "Masque eau généré")
    ...
    def _run_tp1(self):
        ...
        self._run_snap_graph(g_before, label="TP1-BEFORE")
        ...
        self._run_snap_graph(g_after, label="TP1-AFTER")
...
ds_b = gdal.Open(before_aligned if os.path.exists(before_aligned) else before_db)
bd   = ds_b.GetRasterBand(1).ReadAsArray().astype(np.float32)
ad   = ds_a.GetRasterBand(1).ReadAsArray().astype(np.float32)
rows = min(bd.shape[0], ad.shape[0])
cols = min(bd.shape[1], ad.shape[1])
bd, ad = bd[:rows,:cols], ad[:rows,:cols]

diff = bd - ad
nodata_mask = (bd < -200) | (ad < -200)
diff[nodata_mask] = np.nan

def save(path, arr, dtype=gdal.GDT_Float32, nd=None):
    drv = gdal.GetDriverByName("GTiff")
    o   = drv.Create(path, cols, rows, 1, dtype, options=["COMPRESS=LZW"])
    o.SetGeoTransform(ds_a.GetGeoTransform())
    o.SetProjection(ds_a.GetProjection())
    o.GetRasterBand(1).WriteArray(arr)
    if nd is not None: o.GetRasterBand(1).SetNoDataValue(nd)
    o.FlushCache()

save(diff_tif, np.where(np.isnan(diff), -9999, diff), nd=-9999)
...
save(out_mask, mask, gdal.GDT_Byte, 255)
...
for i,(band,ci) in enumerate(zip([stretch(bd),stretch(ad),stretch(ad)],
    [gdal.GCI_RedBand, gdal.GCI_GreenBand, gdal.GCI_BlueBand]), 1):
    o.GetRasterBand(i).WriteArray(band)
    o.GetRasterBand(i).SetColorInterpretation(ci)
o.FlushCache()
...
drv = gdal.GetDriverByName("GTiff")
o = drv.Create(smoothed_tif, ds.RasterXSize, ds.RasterYSize, 1, gdal.GDT_Float32, options=["COMPRESS=LZW"])
o.SetGeoTransform(gt); o.SetProjection(prj)
o.GetRasterBand(1).WriteArray(sm.astype(np.float32)); o.FlushCache(); o = None; ds = None; band = None; data = None; sm = None

# --- B: Reclassification Natural Breaks ---
print("[B] Reclassification...")
from scipy.cluster.vq import kmeans
ds_s = gdal.Open(smoothed_tif)
data = ds_s.GetRasterBand(1).ReadAsArray().astype(np.float64)
valid = data[np.isfinite(data)].flatten()
samp  = valid if len(valid)<500000 else valid[np.random.choice(len(valid),500000,replace=False)]
ctr, _ = kmeans(samp, 2)
thr = float(ctr.mean())
print(f"  Seuil Natural Breaks: {{thr:.4f}}")
reclass_tif = mask_water.replace(".tif","_reclass.tif")
reclass = np.where(~np.isfinite(data), 0, np.where(data > thr, 2, 1)).astype(np.int16)
o = drv.Create(reclass_tif, ds_s.RasterXSize, ds_s.RasterYSize, 1, gdal.GDT_Int16, options=["COMPRESS=LZW"])
o.SetGeoTransform(ds_s.GetGeoTransform()); o.SetProjection(ds_s.GetProjection())
o.GetRasterBand(1).WriteArray(reclass); o.GetRasterBand(1).SetNoDataValue(0); o.FlushCache(); o = None; ds_s = None; data = None; reclass = None
